...
class SampleGroupModel(QAbstractTableModel):
    ...
    def remove_group(self, group: SampleGroup):
        ...
        if group in self.sample_groups:
            # Find the row index of the group
            row = self.sample_groups.index(group)
            self.remove_sample_group(row)
...
class SampleMappingWidget(QWidget):
    ...
    def init_ui(self):
        ...
        # Create audio controls above the piano
        ...
        # Volume slider (half width, right aligned)
        self.volume_slider = QSlider(Qt.Horizontal)
        self.volume_slider.setMinimum(0)
        self.volume_slider.setMaximum(100)
        self.volume_slider.setValue(100)  # Default to full volume
        self.volume_slider.setMaximumWidth(150)  # Half width
        self.volume_slider.valueChanged.connect(self.update_playback_volume)
        ...
        audio_layout.addWidget(self.mute_button)
        
        # Add audio controls to splitter
        splitter.addWidget(audio_widget)
    ...
    def update_playback_volume(self, value):
        """Update the playback volume and label."""
        # Update the volume label
        self.volume_label.setText(f"{value}%")
        
        # Update the volume in the media player if it exists
        if hasattr(self, 'media_player') and self.media_player:
            if hasattr(self.media_player, 'audioOutput'):
                # Convert percentage to linear volume (0.0 to 1.0)
                volume = value / 100.0
                self.media_player.audioOutput().setVolume(volume)
    
    def toggle_mute(self, checked):
        """Toggle mute/unmute for audio playback."""
        if checked:
            # Mute
    ...
    def play_sample(self, note_number):
        """Play the sample associated with the clicked key with pitch adjustment."""
        if note_number in self.visual_keyboard.assigned_samples:
            ...
            if sample_group.samples:
                ...
                # Pass the target note for pitch adjustment
                self.play_sample_file(sample, target_note=note_number)
            else:
    ...
    def try_qmediaplayer_playback(self, sample, target_note=None):
        """Try to play audio using PySide6 QMediaPlayer with optional pitch adjustment."""
        try:
            ...
            # Apply pitch adjustment if target_note is different from sample's root note
            if target_note is not None and target_note != sample.root_note:
                pitch_semitones = target_note - sample.root_note
                # QMediaPlayer doesn't have direct pitch control, but we can use playback rate
                # This is a simplified approach - in a real implementation, you'd want proper pitch shifting
                playback_rate = 2 ** (pitch_semitones / 12.0)  # Convert semitones to playback rate
                self.media_player.setPlaybackRate(playback_rate)
                self.show_status_message(f"Playing: {sample.file_path.name} (pitch adjusted by {pitch_semitones:+d} semitones)")
            else:
    ...
    def play_sample_file(self, sample, target_note=None):
        """Play a sample file directly with optional pitch adjustment."""
        ...
        if self.try_qmediaplayer_playback(sample, target_note):
            return
    ...
    def force_sync_xml(self):
        ...
        # Also trigger the main window's XML update if available
        if hasattr(self, 'main_window') and self.main_window:
            self.main_window.schedule_xml_update()
    ...
    def get_sample_groups(self) -> List[SampleGroup]:
        ...
        # Round robin groups are already integrated into the regular groups
        # through the auto_detect_groups method, so we don't need to add them again
        # This prevents duplicates in the XML generation
...
class RoundRobinManager(QWidget):
    ...
    def setup_ui(self):
        ...
        # Add refresh button (icon only, aligned right)
        self.refresh_btn = QPushButton("🔄")
        self.refresh_btn.setToolTip("Sync with XML Preview")
        ...
        """)
        self.refresh_btn.clicked.connect(self.refresh_xml)
    ...
    def auto_detect_groups(self):
        ...
        if detected_groups > 0:
            # Update existing groups or create new ones for detected groups
            for group_name, group_data in self.round_robin_groups.items():
                if hasattr(self.sample_mapping, 'model'):
                    # Find existing group or create new one
                    existing_group = None
                    for group in self.sample_mapping.model.sample_groups:
                        if group.name == group_name:
                            existing_group = group
                            break
                    ...
                    if existing_group:
                        ...
                        existing_group.seq_mode = group_data['seq_mode']
                        existing_group.seq_length = group_data['seq_length']
                        # Move samples to this group
                        for sample in group_data['samples']:
                            self.sample_mapping.model.add_sample_to_group(sample, existing_group)
    ...
    def set_round_robin_groups(self, groups: dict):
        """Set round robin groups from saved data."""
        self.round_robin_groups = groups
        self.update_groups_tree()
